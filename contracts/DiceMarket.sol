...
contract DiceMarket {
    ...
    function list(uint256 id, uint256 price) public {
        require(price >= (diceContract.getDiceValue(id) + comissionFee), "list price less than creation value + commission fee");
    ...
    function buy(uint256 id) public payable {
        ...
        address payable recipient = address(
            uint160(diceContract.getPrevOwner(id))
        );
    ...
    function withDraw() public {
        if (msg.sender == _owner) msg.sender.transfer(address(this).balance);
    }
}
